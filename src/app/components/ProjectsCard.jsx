...
function ProjectsCard({imgUrl, title, description, gitUrl, previewUrl}) {
  return (
    <div className='my-4 md:w-[360px]'>
        <div className='h-52  rounded-t-lg relative group' style={{background : `url(${imgUrl})`, backgroundSize: 'cover'}}>
            <div className='overlay flex items-center justify-center gap-4 absolute top-0 left-0 w-full h-full bg-[#3f3f42] bg-opacity-0 hidden group-hover:flex group-hover:bg-opacity-80 transition-all duration-500'>
                <Link href={gitUrl} className='  text-slate-300'>
                ...
                <Link href={previewUrl} className=' text-slate-300'>
